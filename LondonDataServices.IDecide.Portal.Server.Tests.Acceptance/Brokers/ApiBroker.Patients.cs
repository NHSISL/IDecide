...
namespace LondonDataServices.IDecide.Portal.Server.Tests.Acceptance.Brokers
{
    public partial class ApiBroker
    {
        ...
        public async ValueTask<List<Patient>> GetSpecificPatientByIdAsync(Guid patientId) =>
            await this.apiFactoryClient.GetContentAsync<List<Patient>>(
                $"{patientsRelativeUrl}?$filter=Id eq {patientId}");

        public async ValueTask<Patient> GetPatientByIdAsync(Guid patientId) =>
            await this.apiFactoryClient
                .GetContentAsync<Patient>($"{patientsRelativeUrl}/{patientId}");

        public async ValueTask<Patient> DeletePatientByIdAsync(Guid patientId) =>
            await this.apiFactoryClient
                .DeleteContentAsync<Patient>($"{patientsRelativeUrl}/{patientId}");
