...
namespace LondonDataServices.IDecide.Manage.Server.Tests.Unit.Controllers.Consumers
{
    public partial class ConsumersControllerTests
    {
        ...
        public async Task ShouldReturnRecordOnGetByIdsAsync()
        {
            ...
            // when
            ActionResult<Consumer> actualActionResult = await consumersController.GetConsumerByIdAsync(inputId);

            // then
            actualActionResult.ShouldBeEquivalentTo(expectedActionResult);

            consumerServiceMock
                .Verify(service => service.RetrieveConsumerByIdAsync(It.IsAny<Guid>()),
                    Times.Once);
